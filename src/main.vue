<template>
    <div class="wp-upload">
        ...
        <div class="wp-upload-sum-progress" :class="{fail: isUploadFail}" v-show="sumProgressStatus.show">
            <div class="wp-upload-sum-progress-inner" :class="{fail: isUploadFail}" :style="{width: sumProgressStatus.width + '%'}"></div>
        </div>
        <!--选择文件-->
        <input class="wp-upload-input-select-file" type="file" :multiple='type == "multiple"' style="display: none" @change="fileSelectChange"/>
        <!--文件本体-->
            ...
            <div class="wp-upload-list-container">
                <div class="wp-upload-list-item" v-for="showItem in fileShowArray">
                    ...
                    <div class="process" v-show="showItem.processStatus.show">
                        <div class="process-inner" :class="{fail : showItem.processStatus.isFail}" :style="{width : showItem.processStatus.width + '%'}"></div>
                    </div>
                    <div class="menus">
                        <div class="file-name">{{showItem.name}}</div>
                        <div class="menu-list">
                            <div class="menu-item" v-if="showItem.show==1 || showItem.show==2 || showItem.show==3" @click="showSource(showItem)">
                                <span class="iconfont icon-fangdajing"></span>
                            </div>
                            <div class="menu-item" @click="deleteFile(showItem.id)">
                ...
                <div class="wp-upload-button-select-file" @click="selectFile" v-show="selectFileShow">
...
<script>
    ...
    import {wpUploadFileTools,wpUploadDataValid,wpUploadFileAlert,wpUploadDataTools,wpUploadFileMessage,wpUploadFileTypeResolving,wpUploadFileShowResolving} from './js/wptools'
    export default {
        name: "wp-upload",
        props:{
            /**
        ...
        methods:{
             ...
             */
            getInputObj() {
                return document.getElementsByClassName("wp-upload-input-select-file")[0];
            },
            /**
             * 选择文件
             */
            ...
            addFileUploadToList(files) {
                ...
                // 回调函数
                this.$emit('select',preFileItemArray,error_numberFileArray,error_sizeFileArray,error_allowFileArray);
            },
            /**
             * 添加一组显示文件
            ...
            upload(params = {},url = "") {
                ...
                if(this.fileShowArray.length <= 0) {
                    // 没有文件可以上擦魂
                ...
                // 上传文件之前的回调
                this.$emit('upload-before',formData);
                // 设置上传失败false
                this.isUploadFail = false;
                // 如果需要总进度条
            ...
            uploadFile(formData,url) {
                ...
                axios.post(url, formData, config).then(res => {
                    ...
                    this.uploadSuccess();
                    this.$emit('upload-after',this.uploadResult);
                }).catch(e =>{
            ...
            uploadFileItem(fileShowItem) {
                ...
                axios.post(this.url, formData, config).then(res => {
                    ...
                    this.$emit('upload-after',uploadResult);
                }).catch(e =>{
                    this.deCreInAutoUploadCount();
                    let uploadResult = {
                        status:500,
